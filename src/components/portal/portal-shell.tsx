                ...
                <div className="max-h-64 overflow-y-auto mt-1">
                  {shortlistedCars.length === 0 ? (
                    ...
                    shortlistedCars.map((car) => (
                      <div key={car.id} className="px-4 py-2.5 hover:bg-gray-50 border-b border-gray-55 last:border-b-0 flex gap-3 items-center group/item relative">
                        <Link 
                          href={`/car/${car.id}`}
                          onClick={() => setActiveDropdown(null)}
                ...
                {user ? (
                  <>
                    ...
                    <Link 
                      href="/account"
                      onClick={() => setActiveDropdown(null)}
                      className="w-full flex items-center gap-2 px-4 py-2 hover:bg-gray-55 text-xs font-bold text-gray-700 text-left transition-colors"
                    >
...
export function PortalShell({
  ...
  return (
    <div className="min-h-screen bg-white text-turo-dark flex flex-col font-sans antialiased">
      <Suspense fallback={
        <header className="z-50 bg-[#300754] text-white sticky top-0 h-28 shadow-lg select-none flex items-center justify-center">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md">
            <button
              ...
              className="mb-3 ml-auto block rounded-full bg-white/15 px-3 py-1 text-xs font-bold text-white cursor-pointer"
            >
              Close
            </button>
